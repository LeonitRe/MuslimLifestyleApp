...
const product = async () => {
  const response = await axios.get('http://localhost:5080/products?category=kids')
...
const filteredProducts = computed(() => {
  return prd.value.filter((p) => {
    ...
    const isCategoryMatch = category === 'kids'
  ...
  <FilterBox>
    <Filter filter="Filter">
      <Item>
        ...
        <li>
          <button type="button" class="btn btn-light" @click="setProductType('Boys')">Boys</button>
        </li>
        <li>
          <button type="button" class="btn btn-light" @click="setProductType('Girls')">
            Girls
          </button>
        </li>
      </Item>
